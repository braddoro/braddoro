...
switch ($s_page_task) {
    case "save":
		...
		if ($s_submit > "") {
			if ($i_ownerID > 0) {
				$s_sql = 'update braddoro.dyn_pos_owners set owner = "'.$s_owner.'", active = '.$i_active.' where ownerID = '.$i_ownerID.';';
			} else {
				if ($s_owner > "") { 
					$s_sql = 'insert into braddoro.dyn_pos_owners (owner, active, publicID) select "'.$s_owner.'", '.$i_active.', uuid();';
				}
			}
			if ($s_sql > "") {
	...
	case "list":
		...
		$s_sql = 'select ownerID, owner, active from braddoro.dyn_pos_owners order by ownerID desc;';
		$q_data = mysql_query($s_sql);
		if (!$q_data) {die_well(__LINE__,mysql_error());}
		$i_fields = mysql_num_fields($q_data);
		echo '<a href="'.$s_fileName.'?ownerID=0&page_task=edit" target="_top"><img src="../../images/Button-Play-16x16.png" border="0" height="12" width="12" title="Add"></a>';
		...
		while ($rowData = mysql_fetch_row($q_data)) {
			...
			echo '<a href="'.$s_fileName.'?ownerID='.$rowData[0].'&page_task=edit" target="_top">';
    ...
    case "edit":
    	echo $s_header;
    	$page_task="save";
		$s_sql = 'select ownerID, owner, active from braddoro.dyn_pos_owners where ownerID = '.$i_ownerID.';';
		$q_data = mysql_query($s_sql);
		if (!$q_data) {die_well(__LINE__,mysql_error());}
		while ($rowData = mysql_fetch_row($q_data)) {
			$i_ownerID = $rowData[0]; 
		...
		echo row_input_text($s_fieldName="ownerID",$i_fieldSize=5,$s_data=$i_ownerID,$left_class="odd",$right_class="even",$s_fieldString="Owner ID",$use_input=0,$use_hidden=1,$value_only=1).$g_break;
		echo row_input_text($s_fieldName="owner",$i_fieldSize=30,$s_data=trim($s_owner),$left_class="odd",$right_class="even",$s_fieldString="Owner").$g_break;
		...
		if ($i_ownerID > 0) {$s_text = "Save";}else{$s_text = "Add";}		
		echo '<tr>'.$g_break;
