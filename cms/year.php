...
.detail0 {width:10px;text-align:right;height:20px;vertical-align:middle;font-weight:bold;}
.detail {text-align:right;height:20px;vertical-align:top;border: 1px solid #999999;}
...
function js_expand(i_joo,s_greg,s_dow) {
	...
	document.getElementById("div_expand").style.top = (screen.height/4)/2;
...
function js_saveMe() {
	...
	if (document.getElementById("overlayID").value > 0) {
		//alert(s_ajax);
		var dada = http_post_request("year_ajax.php",s_ajax);
		//prompt("",dada);
...
$s_sql = "select dateID, overlayID, julianDate, event from cms.dyn_julian_dates where overlayID = $i_overlayID order by julianDate";
$server		= "65.175.107.2:3306";
$username	= "cms_user";
$password	= "alvahugh";
$dbname		= "cms";
$conn=mysql_connect($server, $username, $password);
//if (!$o_conn) {exit(mysql_error());}
$o_sel = mysql_select_db($dbname);
//if (!$o_sel) {exit(mysql_error());}

$sqldb=mysql_select_db($dbname,$conn);
$o_result = mysql_query($s_sql,$conn);
echo "<input type='hidden' name='overlayID' id='overlayID' value='$i_overlayID'>";
echo "<span class='label'>$i_year</span>";
echo "<table class='tabledata' border='0'>";
for ($i_month=1;$i_month<13;$i_month++) {
	...
	echo "<tr>";
	echo "<td class='detail0'>$s_month</td>";
	for ($i_day=1;$i_day<32;$i_day++) {
		...
		if (mysql_num_rows($o_result) > 0) {
			mysql_data_seek($o_result,0);
		}
		while($row = mysql_fetch_object($o_result)) {
			$dateID		= $row->dateID;
		...
		echo "<td id='day_$i_julianYear' title='$i_julianYear' class='detail' style='$s_style$s_bold' onclick='js_expand($i_julianYear,\"$s_greg\",\"$s_dow\");'>";
		if ($i_day <= $i_days) {
			if ($i_day < 10 ) {echo "&nbsp;";}
