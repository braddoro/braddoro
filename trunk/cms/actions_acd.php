...
$i_timeTypeID = set_var('timeTypeID',0);
$s_timeType = set_var('timeType',"");
...
switch ($s_page_task) {
    case "save":
		...
		if ($s_submit > "") {
			if ($i_timeTypeID > 0) {
				$s_sql = 'update cms.cfg_time_types set timeType = "'.$s_timeType.'", active = '.$i_active.' where timeTypeID = '.$i_timeTypeID.';';
			} else {
				if ($s_timeType > "") { 
					$s_sql = 'insert into cms.cfg_time_types (timeType, active) select "'.$s_timeType.'", '.$i_active.';';
				}
			}
			if ($s_sql > "") {
	...
	case "list":
		...
		$s_sql = 'select timeTypeID, timeType, active from cms.cfg_time_types order by timeType;';
		...
		echo '<a href="'.$s_fileName.'?timeTypeID=0&page_task=edit" target="_top"><img src="../../images/Button-Add-16x16.png" border="0" height="12" width="12" title="Add"></a>';
		echo '</th>';
		...
		while ($rowData = mysql_fetch_row($q_data)) {
			...
			echo '<a href="'.$s_fileName.'?timeTypeID='.$rowData[0].'&page_task=edit" target="_top">';
    ...
    case "edit":
    	echo $s_header;
    	$page_task="save";
		$s_sql = 'select timeTypeID, timeType, active from cms.cfg_time_types where timeTypeID = '.$i_timeTypeID.';';
		$q_data = mysql_query($s_sql);
		if (!$q_data) {die_well(__LINE__,mysql_error());}
		while ($rowData = mysql_fetch_row($q_data)) {
			$i_timeTypeID = $rowData[0]; 
			$s_timeType = $rowData[1];
			$i_active = $rowData[2];
		}
		...
		echo row_input_text($s_fieldName="timeTypeID",$i_fieldSize=5,$s_data=$i_timeTypeID,$left_class="odd",$right_class="even",$s_fieldString="Action ID",$use_input=0,$use_hidden=1,$value_only=1).$g_break;
		echo row_input_text($s_fieldName="timeType",$i_fieldSize=30,$s_data=trim($s_timeType),$left_class="odd",$right_class="even",$s_fieldString="Action").$g_break;
		...
		if ($i_timeTypeID > 0) {$s_text = "Save";}else{$s_text = "Add";}		
		echo '<tr>'.$g_break;
